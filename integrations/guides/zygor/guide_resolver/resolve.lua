...
local IsActionableFact = P.IsActionableFact
...
local SelectDirectLiveActionFact = P.SelectDirectLiveActionFact
...
local function IsScenarioNonActionableFact(fact)
    return type(fact) == "table"
        and (fact.action == "scenariogoal" or fact.action == "scenarioend" or fact.action == "scenariostart")
...
local function ResolveScenarioNonActionable(currentFact, facts, rawArrowTitle, targetMapID, targetX, targetY, targetSig)
    ...
    -- scenariostart: prefer nearby click/use helper before objective instruction
    if currentFact.action == "scenariostart" then
        local helperActionFact = SelectQuestObjectiveHelperActionFact(
        ...
        if type(helperActionText) == "string" and helperActionText ~= "" then
            return currentFact.text, helperActionText, "non_actionable_helper_action_fallback"
        end
    end
    ...
    -- scenariogoal/scenariostart: passive subtext falls through after instruction attempt
    if currentFact.action == "scenariogoal" or currentFact.action == "scenariostart" then
        local passiveText, passiveReason = SelectPassiveSubtextFallback(
            facts, currentFact.index, currentFact.index, rawArrowTitle)
        if type(passiveText) == "string" and passiveText ~= "" then
            return currentFact.text, passiveText, passiveReason
...
local LIVE_STALE_SEED_REASONS = {
    turnin_not_offered = true,
    accept_in_log = true,
}
...
local function IsLiveStaleSeedFact(liveCurrentness, fact)
    if type(fact) ~= "table" or fact.orlogic ~= nil then
        return false
    end

    local annotation = GetLiveAnnotation(liveCurrentness, fact)
    return type(annotation) == "table"
        and annotation.secondaryEligible == false
        and LIVE_STALE_SEED_REASONS[annotation.reason] == true
end

local function CollectSameTargetActionableQuestFacts(facts, targetMapID, targetX, targetY, targetSig)
    local results = {}
    for _, fact in ipairs(facts or {}) do
        if IsQuestActionFact(fact)
            and IsActionableFact(fact)
            and fact.orlogic == nil
...
local function ResolveLiveDirectSeedOverride(facts, seedIndex, liveCurrentness, targetMapID, targetX, targetY, targetSig)
    local seedFact = type(seedIndex) == "number" and facts[seedIndex] or nil
    if not IsLiveStaleSeedFact(liveCurrentness, seedFact) then
        return nil
    end

    local directLiveFact = SelectDirectLiveActionFact and select(1, SelectDirectLiveActionFact(
        liveCurrentness,
        CollectSameTargetActionableQuestFacts(facts, targetMapID, targetX, targetY, targetSig)
    ))
    if type(directLiveFact) ~= "table"
        or type(directLiveFact.index) ~= "number"
        or directLiveFact.index == seedIndex
    then
        return nil
    end

    return directLiveFact.index
end

local function HasQuestLogReadyTurninProof(questID)
...
local function HasReadyTurninProof(fact, liveCurrentness)
    if type(fact) ~= "table" or fact.action ~= "turnin" then
        return false
    end

    local annotation = GetLiveAnnotation(liveCurrentness, fact)
    if type(annotation) == "table" and type(annotation.matchKind) == "string" then
        return true
    end

    return HasQuestLogReadyTurninProof(fact.questid)
end

local function CanTraverseReadyTurninCorridorFact(fact, targetMapID, targetX, targetY, targetSig)
    if IsGuidanceFact(fact) or IsInformationalTextFact(fact) then
        return true
    end

    return IsQuestActionFact(fact)
        and fact.orlogic == nil
        and fact.status == "complete"
        and IsSameTargetFact(fact, targetMapID, targetX, targetY, targetSig)
end

local function ResolveReadyTurninSeedOverride(facts, seedIndex, liveCurrentness, targetMapID, targetX, targetY, targetSig)
    local seedFact = type(seedIndex) == "number" and facts[seedIndex] or nil
    if not (seedFact
        and seedFact.action == "accept"
        and IsActionableFact(seedFact)
        and seedFact.orlogic == nil
        and IsSameTargetFact(seedFact, targetMapID, targetX, targetY, targetSig)
    ) then
        return nil
    end

    for i = seedIndex - 1, 1, -1 do
        local fact = facts[i]
        if fact and fact.visible == true and fact.suppressed ~= true then
            if IsQuestActionFact(fact)
                and fact.action == "turnin"
                and fact.status == "incomplete"
                and fact.orlogic == nil
                and IsSameTargetFact(fact, targetMapID, targetX, targetY, targetSig)
            then
                return HasReadyTurninProof(fact, liveCurrentness) and fact.index or nil
            end

            if not CanTraverseReadyTurninCorridorFact(fact, targetMapID, targetX, targetY, targetSig) then
                return nil
            end
        end
    end
...
local function IsVisibleIncompleteQuestTargetFact(fact, targetMapID, targetX, targetY, targetSig)
    return type(fact) == "table"
        and fact.visible == true
        and fact.suppressed ~= true
        and fact.status == "incomplete"
...
local function SelectVisibleQuestSemanticFact(currentFact, facts, rawArrowTitle, targetMapID, targetX, targetY, targetSig)
    if IsVisibleIncompleteQuestTargetFact(currentFact, targetMapID, targetX, targetY, targetSig) then
        return currentFact
    end

    local rawText = NormalizeText(rawArrowTitle)
    for _, fact in ipairs(facts or {}) do
        if IsVisibleIncompleteQuestTargetFact(fact, targetMapID, targetX, targetY, targetSig)
            and rawText ~= nil
            and NormalizeText(fact.text) == rawText
        then
            return fact
        end
    end
...
local function ResolveNonActionable(input, debug)
    ...
    if type(fallbackSeedIndex) == "number" then
        ...
        local headerFact = SelectHeaderFact(facts, resolvedFallbackSeedIndex, resolvedFallbackSeedIndex, false, true)
    ...
    local questSemanticFact = SelectVisibleQuestSemanticFact(
        currentFact,
        facts,
        rawArrowTitle,
        targetMapID,
    ...
    if questSemanticFact then
        local snapshot = BuildSnapshot(
            rawArrowTitle,
            NormalizeText(questSemanticFact.text) or rawArrowTitle,
            nil,
            "normal",
            nil,
            questSemanticFact,
            nil,
            BuildSemanticInfo("quest", nil, questSemanticFact.questid)
        )
        ApplyRoutePresentationPolicy(snapshot, context, currentGoalNum, facts)
        debug.reason = "visible_quest_goal_semantic_fallback"
        debug.snapshot = snapshot
        return snapshot, debug
    end
...
local function ResolveActionable(input, debug)
    ...
    local liveSeedIndex = ResolveLiveDirectSeedOverride(
        facts,
        seedIndex,
        liveCurrentness,
        targetMapID,
    ...
    if type(liveSeedIndex) == "number" then
        seedIndex = liveSeedIndex
    end
    local readyTurninSeedIndex = ResolveReadyTurninSeedOverride(
        facts,
        seedIndex,
        liveCurrentness,
        targetMapID,
    ...
    if type(readyTurninSeedIndex) == "number" then
        seedIndex = readyTurninSeedIndex
    end
    if type(seedIndex) ~= "number" then
