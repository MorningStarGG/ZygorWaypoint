...
local function ScanHeader(facts, startIndex, direction, allowInformationalTextBridge, allowPassiveSubtextBridge)
    return WalkDirectionalVisibleFacts(facts, startIndex, direction, function(fact)
        ...
        if allowPassiveSubtextBridge and IsPassiveSubtextCandidateFact(fact) then
            return "continue"
        end
        return nil
    end)
...
local function SelectHeaderFact(facts, startIndex, endIndex, allowInformationalTextBridge, allowPassiveSubtextBridge)
    local backward = ScanHeader(facts, startIndex, -1, allowInformationalTextBridge, allowPassiveSubtextBridge)
    local forward = ScanHeader(facts, endIndex, 1, allowInformationalTextBridge, allowPassiveSubtextBridge)
...
local function SelectObjectiveInteractionChainHeaders(facts, seedIndex, titleText)
    ...
    local secondaryHeaderFact = SelectHeaderFact(facts, seedIndex, seedIndex, true, true)
