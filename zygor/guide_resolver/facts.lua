...
local function BuildFacts(step)
    ...
    for i, goal in ipairs(step.goals) do
        ...
        facts[i] = {
            ...
            visible = visible,
            status = status,
            questid = GetGoalQuestID(goal),
            questTitle = GetGoalQuestTitle(goal),
            npcid = GetGoalNPCID(goal),
            orlogic = type(goal) == "table" and type(goal.orlogic) == "number" and goal.orlogic or nil,
            mapID = mapID,
            x = x,
            y = y,
            text = text,
...
local function GetCachedFacts(step)
    ...
    if resolverState.factsCacheStep == step
        and resolverState.factsCacheEpoch == factsEpoch
        and type(resolverState.factsCacheValue) == "table"
    then
        return resolverState.factsCacheValue
    end

    local facts = BuildFacts(step)
...
resolveCanonicalGuideGoal = function(step)
    ...
    local result = resolveCanonicalGoalFromFacts(GetCachedFacts(step), rawGoalNum)
    canonicalCacheStep = step
    canonicalCacheRawGoalNum = rawGoalNum
    canonicalCacheFactsEpoch = factsEpoch
...
local function MatchesSnapshotCacheKey(context, step, currentGoalNum, rawArrowTitle, targetMapID, targetX, targetY, targetSig)
    ...
    return step == snapshotCacheStep
        ...
        and (context and context.source or nil) == snapshotCacheSource
...
local function StoreSnapshotCacheKey(context, step, currentGoalNum, rawArrowTitle, targetMapID, targetX, targetY, targetSig)
    ...
    snapshotCacheSource = context and context.source or nil
