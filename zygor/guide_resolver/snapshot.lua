...
local function DetectExplicitTravelTypeForSemantics(action, rawArrowTitle, detailText, npcid, mapID, x, y)
    return NS.ClassifyTravelSemantics(action, npcid, mapID, x, y, rawArrowTitle, detailText)
end
...
local function ApplyRoutePresentationPolicy(snapshot, context, currentGoalNum, facts)
    ...
    if context.legKind == "carrier" and snapshot.semanticKind == "quest" and routeTravelType == nil then
