...
local function EnsureThemeTextures(theme, button)
    ...
    if not theme.specularTexture then
        ...
        theme.specularTexture:SetBlendMode("ADD")
    end
end

local function IsValidThemeSprite(sprite)
    return type(sprite) == "table"
        and type(sprite.spr_w) == "number"
        and type(sprite.spr_h) == "number"
        and type(sprite.img_w) == "number"
        and type(sprite.img_h) == "number"
        and type(sprite.spritecount) == "number"
end

local function IsValidThemeDef(themeDef)
    return type(themeDef) == "table"
        and type(themeDef.themeKey) == "string"
        and type(themeDef.displayName) == "string"
        and type(themeDef.skinDir) == "string"
        and IsValidThemeSprite(themeDef.sprite)
end
...
local function BuildTomTomZygorTheme(themeDef)
    ...
    function theme:NavigationArrow_OnUpdate(elapsed, angle)
        ...
        if left ~= self.lastNavLeft or right ~= self.lastNavRight
            or top ~= self.lastNavTop or bottom ~= self.lastNavBottom
        then
...
local function RegisterTomTomThemes()
    ...
    for _, themeDef in pairs(ZYGOR_THEME_DEFS) do
        if IsValidThemeDef(themeDef) then
