...
local QuestTagType = Enum.QuestTagType or {}
...
local QUEST_TAG_TYPE_FALLBACKS = {}
...
local function IsBattlePetQuest(questTagID, questTagName, questType, questTypeName)
    return IsEnumValue(QuestTag, questTagID, "BattlePet", QUEST_TAG_FALLBACKS)
        or IsEnumValue(QuestTag, questType, "BattlePet", QUEST_TAG_FALLBACKS)
        or IsQuestTagNameMatch(questTagName, "BattlePet")
        or IsQuestTagNameMatch(questTagName, "Battle Pet World Quest")
        or IsQuestTagNameMatch(questTypeName, "BattlePet")
        or IsQuestTagNameMatch(questTypeName, "Battle Pet World Quest")
end

local function IsWorldBossQuest(worldQuestType, worldQuestTypeName)
    return IsEnumValue(QuestTagType, worldQuestType, "Raid", QUEST_TAG_TYPE_FALLBACKS)
        or IsEnumValue(QuestTagType, worldQuestType, "WorldBoss", QUEST_TAG_TYPE_FALLBACKS)
        or IsQuestTagNameMatch(worldQuestTypeName, "Raid")
        or IsQuestTagNameMatch(worldQuestTypeName, "WorldBoss")
        or IsQuestTagNameMatch(worldQuestTypeName, "World Boss")
end

local function IsRacingQuest(questTagID, questTagName, questType, questTypeName, worldQuestType, worldQuestTypeName)
    return IsEnumValue(QuestTag, questTagID, "DragonRiderRacing", QUEST_TAG_FALLBACKS)
        or IsEnumValue(QuestTag, questType, "DragonRiderRacing", QUEST_TAG_FALLBACKS)
        or IsEnumValue(QuestTagType, worldQuestType, "DragonRiderRacing", QUEST_TAG_TYPE_FALLBACKS)
        or IsQuestTagNameMatch(questTagName, "DragonRiderRacing")
        or IsQuestTagNameMatch(questTagName, "Dragonrider Racing")
        or IsQuestTagNameMatch(questTypeName, "DragonRiderRacing")
        or IsQuestTagNameMatch(questTypeName, "Dragonrider Racing")
        or IsQuestTagNameMatch(worldQuestTypeName, "DragonRiderRacing")
        or IsQuestTagNameMatch(worldQuestTypeName, "Dragonrider Racing")
end

local function IsGenericWorldQuest(classification, questTagID, questTagName, questType, questTypeName, worldQuestType)
    return IsEnumValue(QuestClassification, classification, "WorldQuest", QUEST_CLASSIFICATION_FALLBACKS)
        or IsEnumValue(QuestTag, questTagID, "WorldQuest", QUEST_TAG_FALLBACKS)
        or IsEnumValue(QuestTag, questType, "WorldQuest", QUEST_TAG_FALLBACKS)
        or IsQuestTagNameMatch(questTagName, "WorldQuest")
        or IsQuestTagNameMatch(questTagName, "World Quest")
        or IsQuestTagNameMatch(questTypeName, "WorldQuest")
        or IsQuestTagNameMatch(questTypeName, "World Quest")
        or worldQuestType ~= nil
end

local function ResolveBonusObjectiveQuestSource(classification, classificationName, questTagID, questTagName, questType, questTypeName)
    if IsEnumValue(QuestClassification, classification, "BonusObjective", QUEST_CLASSIFICATION_FALLBACKS)
        or IsQuestTagNameMatch(classificationName, "BonusObjective")
        or IsQuestTagNameMatch(classificationName, "Bonus Objective")
    then
        return "blizzard-classification"
    end
    if questTagID ~= nil
        and (IsQuestTagNameMatch(questTagName, "BonusObjective") or IsQuestTagNameMatch(questTagName, "Bonus Objective"))
    then
        return "blizzard-tag-info"
    end
    if questType ~= nil
        and (IsQuestTagNameMatch(questTypeName, "BonusObjective") or IsQuestTagNameMatch(questTypeName, "Bonus Objective"))
    then
        return "blizzard-quest-type"
    end
    return nil
end
...
local function ResolveQuestTypeDetails(questID)
    ...
    local questTagInfo, questTagID, questTagName = GetQuestTagDetails(questID)
    ...
    local worldQuestType = type(questTagInfo) == "table" and questTagInfo.worldQuestType or nil
    local worldQuestTypeName = GetEnumValueName(QuestTagType, worldQuestType)
        or GetFallbackValueName(QUEST_TAG_TYPE_FALLBACKS, worldQuestType)
    local classificationName = GetEnumValueName(QuestClassification, classification)
    ...
    if IsWorldBossQuest(worldQuestType, worldQuestTypeName) then
        typeKey = "WorldBoss"
        typeSource = "blizzard-world-quest-type"
    elseif IsBattlePetQuest(questTagID, questTagName, questType, questTypeName) then
        typeKey = "BattlePet"
        typeSource = questTagID ~= nil
            and "blizzard-tag-info"
            or (questType ~= nil and "blizzard-quest-type" or "blizzard-tag-name")
    elseif IsRacingQuest(questTagID, questTagName, questType, questTypeName, worldQuestType, worldQuestTypeName) then
        typeKey = "Racing"
        typeSource = worldQuestType ~= nil
            and "blizzard-world-quest-type"
            or (questTagID ~= nil and "blizzard-tag-info" or "blizzard-quest-type")
    elseif subtype == "Daily" then
        typeKey = "Daily"
    ...
    elseif IsCallingQuest(questID, classification) then
        ...
        local bonusObjectiveSource = ResolveBonusObjectiveQuestSource(
            classification,
            classificationName,
            questTagID,
            questTagName,
        ...
        if bonusObjectiveSource then
            typeKey = "BonusObjective"
            typeSource = bonusObjectiveSource
        elseif IsGenericWorldQuest(classification, questTagID, questTagName, questType, questTypeName, worldQuestType) then
            typeKey = "WorldQuest"
            typeSource = questTagID ~= nil
                and "blizzard-tag-info"
                or (worldQuestType ~= nil and "blizzard-world-quest-type" or "blizzard-classification")
    ...
    local details = {
        ...
        worldQuestTypeName = worldQuestTypeName,
