...
local function StoreDefaultFontStringColor(fontString)
    if not fontString or fontString.__zwpDefaultTextColor then
        return
    end

    local r, g, b, a = fontString:GetTextColor()
    fontString.__zwpDefaultTextColor = {
        r = r or 1,
        g = g or 1,
        b = b or 1,
        a = a or 1,
...
local function ApplyContextIconStyle(contextIcon, displayMode)
    ...
    if contextIcon.Image then
        local containerSize = contextIcon.__zwpContainerSize or 0
        local size = contextIcon.__zwpIconSizeOverride or GetContextGlyphDisplaySize(containerSize, showDiamond)
        local offsetX = contextIcon.__zwpIconOffsetX or 0
        local offsetY = contextIcon.__zwpIconOffsetY or 0
        if size > 0 then
    ...
    contextIcon.__zwpDisplayMode = mode
end
...
local function SetContextIconSpec(contextIcon, spec)
    ...
    local containerSize = contextIcon.__zwpContainerSize
    contextIcon.__zwpIconSizeOverride = ResolveContextGlyphSizeOverride(spec, containerSize)
    contextIcon.__zwpIconOffsetX = ResolveContextGlyphOffset(spec and spec.iconOffsetX, spec, containerSize)
    contextIcon.__zwpIconOffsetY = ResolveContextGlyphOffset(spec and spec.iconOffsetY, spec, containerSize)
    ...
    ApplyContextIconStyle(contextIcon, contextIcon.__zwpDisplayMode)
end
...
local function SetHoverHandlers(frame)
    if not frame or frame.__zwpHoverHooked then
        return
    end
...
local function EnsureDriverRoot()
    if not overlay.driver then
        local driver = CreateFrame("Frame", "ZWPWorldOverlayDriver", WorldFrame)
        driver:SetSize(1, 1)
    ...
    if not overlay.root then
        local root = CreateFrame("Frame", "ZWPWorldOverlayRoot", WorldFrame)
...
local function EnsureWaypointFrame()
    ...
    local waypoint = CreateFrame("Frame", "ZWPWorldOverlayWaypoint", overlay.root)
...
local function EnsurePinpointFrame()
    ...
    local pinpoint = CreateFrame("Frame", "ZWPWorldOverlayPinpoint", overlay.root)
    local defaultSpec = GetDefaultPinpointPanelSpec()
    ...
    pinpoint:Hide()
    pinpoint.__zwpPlaqueType = plaqueType
    pinpoint.__zwpPlaqueSpec = plaqueSpec
...
local function EnsureNavigatorFrame()
    ...
    local navigator = CreateFrame("Frame", "ZWPWorldOverlayNavigator", overlay.root)
...
local function ResetFrameTextCaches()
    ...
    if overlay.pinpoint then
        overlay.pinpoint.__zwpTitleText = nil
        overlay.pinpoint.__zwpSubtextText = nil
    end
end
...
M.SwapPinpointPlaqueType = function(plaqueType)
    ...
    if pinpoint.__zwpPlaqueType == plaqueType then return end
    plaqueType = plaqueType or C.WORLD_OVERLAY_PLAQUE_DEFAULT
    Plaques.UpdatePanel(pinpoint.Panel, plaqueType)
    pinpoint.__zwpPlaqueSpec = pinpoint.Panel.__zwpPlaqueSpec or Plaques.GetSpec(plaqueType)
    ApplyPinpointTextHostAnchors(pinpoint)
