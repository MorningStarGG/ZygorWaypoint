...
local BEACON_COLUMN_FADE_DURATION = CFG.BEACON_COLUMN_FADE_DURATION or 0.4
local PINPOINT_ARROW_COUNT = 3

local _adjustedBottomCapLayout = {}
...
local function ResolvePinpointPanelWidth(frame)
    ...
    local titleRescueWidth = ResolvePinpointTextRescueWidth(
        ...
        frame.Title,
        frame.TitleMeasure,
        frame.__awpTitleText or frame.Title:GetText(),
        currentWidth,
        resolvedHeight
    )
    ...
    local subtextRescueWidth = ResolvePinpointTextRescueWidth(
        ...
        frame.__awpSubtextText or frame.Subtext:GetText(),
        currentWidth,
        resolvedHeight
    )
...
local function SetBeaconLayerBottom(texture, parent, offsetX, offsetY)
    ...
    local prevX = texture.__awpBottomX
    local prevY = texture.__awpBottomY
    local prevParent = texture.__awpBottomParent
    ...
    texture.__awpBottomParent = parent
    texture.__awpBottomX = offsetX
    texture.__awpBottomY = offsetY
end
...
local function SetBeaconLayerCenter(texture, parent, offsetX, offsetY)
    ...
    local prevX = texture.__awpCenterX
    local prevY = texture.__awpCenterY
    local prevParent = texture.__awpCenterParent
    ...
    texture.__awpCenterParent = parent
    texture.__awpCenterX = offsetX
    texture.__awpCenterY = offsetY
end
...
local function SetBeaconLayerSize(texture, width, height)
    ...
    local prevW = texture.__awpWidth
    local prevH = texture.__awpHeight
    if prevW and prevH
    ...
    texture.__awpWidth = width
    texture.__awpHeight = height
end
...
local function RestoreFontStringDefaultColor(fontString)
    ...
    local color = fontString.__awpDefaultTextColor
    if not color then
        return
    end
...
local function GetPinpointHostContextGapY()
    ...
    local manualGap = _settings.worldOverlayPinpointManualVerticalGap or PINPOINT_HOST_CONTEXT_GAP_Y
    local manualGapDefault = 200
    local gap = manualGap

    if autoAdjust then
        -- Preserve the existing auto-camera height at default settings, while
        -- letting the manual vertical gap slider act as a user adjustment on top
        -- of the auto baseline.
        gap = math.max(0, PINPOINT_HOST_CONTEXT_GAP_Y + (manualGap - manualGapDefault))
    end
    ...
    if taper > 0 then
        local taperMaxReduction = gap * 0.5
        gap = math.max(0, gap - (taperMaxReduction * taper))
    end
...
local function GetPinpointAnchorOffsetY(offsetY)
    ...
    return (GetPinpointHostContextGapY() - contextCenterFromFrameBottom) + (offsetY or 0)
end
...
local function ApplyWaypointAnchor(extraOffsetY)
    ...
    local offsetY = (_settings.worldOverlayWaypointOffsetY or 0) + (extraOffsetY or 0)
    local useAnchorFrame = derived.anchorFrame and derived.anchorFrame:IsShown()
    if useAnchorFrame then
...
local _beaconVisualStateScratch = {}

local function ResolveWaypointBeaconVisualState(elapsed)
    local cache = _frameCache.waypoint
    local beaconStyle = _settings.worldOverlayBeaconStyle or C.WORLD_OVERLAY_BEACON_STYLE_BEACON
    local showBeacon = beaconStyle ~= C.WORLD_OVERLAY_BEACON_STYLE_OFF and overlay.waypoint and overlay.waypoint.Beacon ~= nil
    local beaconBaseOnly = beaconStyle == C.WORLD_OVERLAY_BEACON_STYLE_BASE

    if not beaconBaseOnly and beaconStyle == C.WORLD_OVERLAY_BEACON_STYLE_DISTANCE then
        local targetDistance = type(GetTargetDistance) == "function" and GetTargetDistance() or nil
        local baseDistance = tonumber(_settings.worldOverlayBeaconBaseDistance) or 0
        beaconBaseOnly = type(targetDistance) == "number"
            and targetDistance > 0
            and targetDistance <= baseDistance
    end

    local targetColumnAlpha = beaconBaseOnly and 0 or 1
    local columnAlpha = targetColumnAlpha
    if showBeacon then
        local cachedColumnAlpha = cache.beaconColumnFadeAlpha
        if type(cachedColumnAlpha) ~= "number" then
            cachedColumnAlpha = targetColumnAlpha
        end
        if cachedColumnAlpha ~= targetColumnAlpha and elapsed and elapsed > 0 then
            local rate = elapsed / BEACON_COLUMN_FADE_DURATION
            if targetColumnAlpha > cachedColumnAlpha then
                cachedColumnAlpha = math.min(targetColumnAlpha, cachedColumnAlpha + rate)
            else
                cachedColumnAlpha = math.max(targetColumnAlpha, cachedColumnAlpha - rate)
            end
        end
        cache.beaconColumnFadeAlpha = cachedColumnAlpha
        columnAlpha = cachedColumnAlpha
    else
        cache.beaconColumnFadeAlpha = nil
        columnAlpha = 0
    end

    local waypointOffsetY = _settings.worldOverlayWaypointOffsetY or 0
    local baseOffsetY = _settings.worldOverlayBeaconBaseOffsetY or 0
    local bottomOffsetAdjust = (1 - columnAlpha) * (baseOffsetY - waypointOffsetY)
    local effectiveBeaconStyle = showBeacon
        and (columnAlpha > 0 and C.WORLD_OVERLAY_BEACON_STYLE_BEACON or C.WORLD_OVERLAY_BEACON_STYLE_BASE)
        or C.WORLD_OVERLAY_BEACON_STYLE_OFF

    local state = _beaconVisualStateScratch
    state.beaconAlpha = _settings.worldOverlayBeaconOpacity
    state.beaconStyle = beaconStyle
    state.bottomOffsetAdjust = bottomOffsetAdjust
    state.columnAlpha = columnAlpha
    state.effectiveBeaconStyle = effectiveBeaconStyle
    state.showBeacon = showBeacon
    state.showColumn = columnAlpha > 0
    return state
end

local function UpdateWaypointVisual(alpha, iconScale, beaconMaskScale, elapsed, beaconState)
    local frame = overlay.waypoint
    if not frame then
        return
    end
    ...
    beaconState = beaconState or ResolveWaypointBeaconVisualState(elapsed)
    local showBeacon = beaconState.showBeacon
    local showColumn = beaconState.showColumn
    local effectiveBeaconStyle = beaconState.effectiveBeaconStyle
    local columnAlpha = beaconState.columnAlpha
    local bottomOffsetAdjust = beaconState.bottomOffsetAdjust
    local beaconAlpha = beaconState.beaconAlpha
    ApplyWaypointAnchor(bottomOffsetAdjust)
    if frame.Beacon and frame.Beacon:IsShown() ~= showBeacon then
        frame.Beacon:SetShown(showBeacon)
    end

    overlay.currentBottomOffsetAdjust = bottomOffsetAdjust

    if frame.ContextIcon and cache.contextIconOffsetY ~= 0 then
        frame.ContextIcon:ClearAllPoints()
        frame.ContextIcon:SetPoint("CENTER", frame, "CENTER", 0, 0)
        cache.contextIconOffsetY = 0
    end

    if frame.Footer and cache.footerOffsetY ~= -1 then
        frame.Footer:ClearAllPoints()
        frame.Footer:SetPoint("TOP", frame, "BOTTOM", 0, -1)
        cache.footerOffsetY = -1
    end

    if frame.Beacon and cache.beaconFrameOffsetY ~= WAYPOINT_BEACON_OFFSET_Y then
        frame.Beacon:ClearAllPoints()
        frame.Beacon:SetPoint("BOTTOM", frame, "CENTER", 0, WAYPOINT_BEACON_OFFSET_Y)
        cache.beaconFrameOffsetY = WAYPOINT_BEACON_OFFSET_Y
    end
    ...
    if cache.beaconStyle ~= effectiveBeaconStyle then
        cache.beaconStyle = effectiveBeaconStyle
    end
    ...
    local now = GetTime()
    if showColumn then
        SetBeaconLayerAlpha(frame.Beacon.Core, beaconAlpha * WAYPOINT_BEACON_ALPHA_MULTIPLIERS.core * columnAlpha)
        SetBeaconLayerAlpha(
            frame.Beacon.Glow,
            beaconAlpha * columnAlpha * GetPulseAlpha(
                now,
    ...
    local bottomCapSrc = WAYPOINT_BEACON_LAYOUT.bottomCap
    _adjustedBottomCapLayout.width = bottomCapSrc.width
    _adjustedBottomCapLayout.height = bottomCapSrc.height
    _adjustedBottomCapLayout.offsetX = bottomCapSrc.offsetX
    _adjustedBottomCapLayout.offsetY = bottomCapSrc.offsetY
    ...
    UpdateBottomCapCore(frame.Beacon.BottomCap, frame, _adjustedBottomCapLayout, now, beaconAlpha)

    UpdateBottomCapFlame(
        frame.Beacon.BottomCapFlameA,
        frame,
        _adjustedBottomCapLayout,
        flameProgressA,
        bottomCapBaseAlpha,
    ...
    if showColumn then
        local veilBaseAlpha = beaconAlpha * WAYPOINT_BEACON_ALPHA_MULTIPLIERS.sideVeil * columnAlpha
...
local function FindPinpointAnimationSpec(frame, animType)
    ...
    local plaqueType = frame.__awpPlaqueType or C.WORLD_OVERLAY_PLAQUE_DEFAULT
...
local function UpdateWaypointFrame(iconSpec, title)
    ...
    ApplyWaypointAnchor(ResolveWaypointBeaconVisualState(0).bottomOffsetAdjust)
...
local function UpdatePinpointFrame(iconSpec, title, subtext)
    ...
    if showPlaque then
        ...
        if frame.__awpTitleText ~= shownTitle then
            frame.Title:SetText(shownTitle)
            frame.__awpTitleText = shownTitle
        end
        if frame.__awpSubtextText ~= shownSubtext then
            frame.Subtext:SetText(shownSubtext)
            frame.__awpSubtextText = shownSubtext
        end
...
local function RenderNativeOverlayVisuals(elapsed)
    ...
    if transition.active and IsWaypointPinpointTransition(transition.fromMode, transition.toMode) then
        ...
        -- Incorporate the live base-offset so the pinpoint diamond starts/ends at the
        local beaconState = ResolveWaypointBeaconVisualState(elapsed)
        local currentOffset = beaconState.bottomOffsetAdjust or 0
        if transition.fromMode == "waypoint" and transition.toMode == "pinpoint" then
            ...
            pinpointOffset = (GetDiamondAlignOffset() + currentOffset) * (1 - pinpointMoveProgress)
        else
            ...
            pinpointOffset = (GetDiamondAlignOffset() + currentOffset) * pinpointMoveProgress
        end

        ShowFrameSet(true, true, showNav)
        UpdateWaypointVisual(waypointAlpha, waypointIconScale, waypointMaskScale, elapsed, beaconState)
        ...
        if progress >= 1 then
            ...
            if completedMode == "waypoint" then
                ...
                UpdateWaypointVisual(_settings.worldOverlayWaypointOpacity * hoverMultiplier, 1,
                    WAYPOINT_BEACON_MASK_SHOWN_SCALE, elapsed)
            else
    ...
    if derived.mode == "waypoint" then
        ShowFrameSet(true, false, showNav)
        UpdateWaypointVisual(_settings.worldOverlayWaypointOpacity * hoverMultiplier, 1, WAYPOINT_BEACON_MASK_SHOWN_SCALE, elapsed)
        if navIsFading then
            UpdateNavigatorFrame(iconSpec, navFadeAlpha * _settings.worldOverlayNavigatorOpacity * hoverMultiplier)
        end
        return
