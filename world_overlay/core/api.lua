...
local InvalidateQuestIconSpecCache = M.InvalidateQuestIconSpecCache
...
function NS.InvalidateNativeOverlayQuestCaches(questID)
    ...
    if type(InvalidateQuestIconSpecCache) == "function" then
        InvalidateQuestIconSpecCache(questID)
    end
...
function NS.SyncNativeWorldOverlay(uid, mapID, x, y, title, source, kind, contentSnapshot)
    ...
    EndChurnPhase("phaseNativeSyncKB", phase)
end
...
function NS.UpdateNativeWorldOverlay()
    ...
    if not target.active then
        NS.ClearNativeWorldOverlay()
        EndChurnPhase("phaseNativeUpdateKB", phase)
        return
    end
    EnsureDriverRoot()
